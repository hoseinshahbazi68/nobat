...
@Component({
  selector: 'app-admin-change-user-password-dialog',
  template: `
    <div class="dialog-container">
      <div class="dialog-header">
        <h2>تغییر کلمه عبور کاربر: {{ user?.firstName }} {{ user?.lastName }}</h2>
      </div>
...
export class AdminChangeUserPasswordDialogComponent implements OnInit {
  ...
  onSave() {
    if (this.changePasswordForm.valid && !this.isLoading && this.user?.id) {
      this.isLoading = true;
      const formValue = this.changePasswordForm.value;

      this.userService.changeUserPassword(this.user.id, formValue.newPassword).subscribe({
        next: () => {
          this.snackbarService.success('کلمه عبور کاربر با موفقیت تغییر کرد', 'بستن', 3000);
